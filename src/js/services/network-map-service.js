portfolio.factory('networkMapService', [
	'$http',
	function($http){

		function renderSparkNetworkMap(){
			var width, height, //Map dimensions
				scale = 1, //initial zoom scale
				drag, //Drag nodes without panning screen
				...
				campaignid = 0,
				drawid = 0,
				zoom = d3.behavior.zoom() //zoom
					.scaleExtent([.02, 5])
					.on("zoom", zoomed);; //Campaign and draw initilization (Dan note= will get this from set campaign ID.)
			...
			function displayMap(campaignID, drawID) { //this did have no input
				...
				svg = d3.select("#project-modal").append("svg") //Insert the svg element inside the element with id "networkmap"	
				...
				$http.get('resources/json/network-map.json').then(response => {
					...
					function buildRoundsList(totalRounds) { //Build the dropdown menu for selecting the rounds
						$("#rounds-list").empty(); //first clear the list
					...
					if (json["error"] != undefined) { //catch when getJSON returns an error, i.e. NO links exist for this campaign (maybe no matches have happened)
						...
						function updateMap(links, drawid) {
							...
							if ($('#matchTable tbody').is(':empty') && !(json['error'] == undefined)) {
								$('#matchTable thead').html("<tr><th>" + json["error"] + "</th></tr>");
							} else 
		...
		return {
			renderSparkNetworkMap: () => {renderSparkNetworkMap();}
		}
